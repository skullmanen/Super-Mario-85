...
public class Camera {
    ...
    public void update(GameContainer gc, GameManager gm, float dt) {
        ...
        if (offY < 0)
            offY = 0;
        if (offX + gc.getWidth() > gm.getMap().getWidth())
            offX = gm.getMap().getWidth() - gc.getWidth();
        if (offY + gc.getHeight() > gm.getMap().getHeight())
            offY = gm.getMap().getHeight() - gc.getHeight();
        // }
